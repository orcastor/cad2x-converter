...
#  include <QtGui/private/qwinrtfontdatabase_p.h>
...
QPlatformFontDatabase* QFontDatabase::instance() {
    static QPlatformFontDatabase* inst = nullptr;
    if (!inst) {
#if defined(Q_OS_WINRT)
        inst = new QWinRTFontDatabase;
#elif defined(Q_OS_WIN)
#  if QT_CONFIG(freetype)
        inst = new QWindowsFontDatabaseFT;
#  endif // freetype
        inst = new QWindowsFontDatabase;
#elif defined(Q_OS_DARWIN)
#  if QT_CONFIG(freetype)
        inst = new QCoreTextFontDatabaseEngineFactory<QFontEngineFT>;
#  endif // freetype
        inst = new QCoreTextFontDatabaseEngineFactory<QCoreTextFontEngine>;
#elif QT_CONFIG(freetype)
        inst = new QFreeTypeFontDatabase;
#else
        inst = new QGenericUnixFontDatabase;
#endif
    }
    return inst;
}
...
void QtFontFamily::ensurePopulated()
{
    ...
    QFontDatabase::instance()->populateFamily(name);
...
void QFontDatabasePrivate::invalidate()
{
    ...
    free();
    QFontDatabase::instance()->invalidate();
}
...
static void initFontDef(const QtFontDesc &desc, const QFontDef &request, QFontDef *fontDef, bool multi)
{
    ...
    if (desc.style->smoothScalable
        || QFontDatabase::instance()->fontsAlwaysScalable()
...
static QStringList fallbacksForFamily(const QString &family, QFont::Style style, QFont::StyleHint styleHint, QChar::Script script)
{
    ...
    QStringList retList = QFontDatabase::instance()->fallbacksForFamily(family,style,styleHint,script);
...
static void initializeDb()
{
    ...
    if (!db->count)
        QFontDatabase::instance()->populateFontDatabase();
                              ...
                              QtFontStyle *style, QtFontSize *size)
{
    ...
    Q_ASSERT(size);
    QPlatformFontDatabase *pfdb = QFontDatabase::instance();
...
static void registerFont(QFontDatabasePrivate::ApplicationFont *fnt)
{
    ...
    fnt->families = QFontDatabase::instance()->addApplicationFont(fnt->data,fnt->fileName);
...
{
    if (QFontDatabase::instance()->fontsAlwaysScalable())
...
{
    return QFontDatabase::instance()->standardSizes();
}
...
{
    return QFontDatabase::instance()->isPrivateFontFamily(family);
}
...
QFontEngine *QFontDatabase::findFont(const QFontDef &request, int script)
{
    ...
    if (index < 0 && QFontDatabase::instance()->populateFamilyAliases()) {
...
{
    return QFontDatabase::instance()->resolveFontFamilyAlias(family);
}
