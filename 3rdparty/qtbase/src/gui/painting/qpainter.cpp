...
void QPainterPrivate::updateEmulationSpecifier(QPainterState *s)
{
    ...
    if (s->state() & (QPaintEngine::DirtyPen | QPaintEngine::DirtyBrush | QPaintEngine::DirtyHints)) {
        ...
        if (penBrush.style() == Qt::TexturePattern)
            penTextureAlpha = penBrush.textureImage().hasAlphaChannel();
        bool brushTextureAlpha = false;
        if (s->brush.style() == Qt::TexturePattern) {
            brushTextureAlpha = s->brush.textureImage().hasAlphaChannel();
        }
...
static QImage generateWavyImage(qreal maxRadius, const QPen &pen)
{
    ...
    QImage img = QImage(width, radius * 2, QImage::Format_ARGB32);
                                   ...
                                   const QTextCharFormat &charFormat)
{
    ...
    if (underlineStyle == QTextCharFormat::WaveUnderline) {
        ...
        const QImage wave = generateWavyImage(qMin(qMax(underlineOffset, pen.widthF()), maxHeight / qreal(2.)), pen);
                    ...
                    QPainter *painter)
{
    ...
    tf = visualAlignment(layout_direction, QFlag(tf));
