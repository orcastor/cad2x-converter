...
static QAtomicPointer<QColorProfile> m_a8ColorProfile;
static QAtomicPointer<QColorProfile> m_a32ColorProfile;

QColorProfile *QColorProfile::colorProfileForA8Text()
{
#ifdef Q_OS_WIN
    const qreal fontSmoothingGamma = 2.31;
    QColorProfile *result = m_a8ColorProfile.load();
    if (!result){
        QColorProfile *cs = QColorProfile::fromGamma(fontSmoothingGamma); // This is a hard-coded thing for Windows text rendering
...
QColorProfile *QColorProfile::colorProfileForA32Text()
{
    const qreal fontSmoothingGamma = 1.7;
