...
void RS_Ellipse::draw(RS_Painter* painter, RS_GraphicView* view, double& offset) {
	if(isEllipticArc()==false){
        RS_Ellipse arc(*this);
        arc.setAngle2(2.*M_PI);
        arc.setReversed(false);
        arc.draw(painter,view,offset);
        return;
    }
...
//    std::cout<<"crossPoints.size()="<<crossPoints.size()<<std::endl;
    ...
    if( crossPoints.size() >= 2) {
		for(size_t i=1;i<crossPoints.size();i+=2){
            ...
            arc.drawVisible(painter,view,offset);
        }
        return;
    }
...
void RS_Ellipse::drawVisible(RS_Painter* painter, RS_GraphicView* view, double& offset) {
...
//    std::cout<<*this<<std::endl;
    ...
    // Adjust dash offset
    updateDashOffset(*painter, *view, offset);
