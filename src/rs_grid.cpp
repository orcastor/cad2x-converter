...
void RS_Grid::updatePointArray() {
	...
	bool scaleGrid = true;//(bool)RS_SETTINGS->readNumEntry("/ScaleGrid", 1);
	// get grid setting
	RS_Vector userGrid;
	if (graphic) {
		...
		isometric = 0;//(bool)RS_SETTINGS->readNumEntry("/IsometricGrid", 0);
		crosshairType=static_cast<RS2::CrosshairType>(0);//RS_SETTINGS->readNumEntry("/CrosshairType",0));
		userGrid.x = -1.;//RS_SETTINGS->readEntry("/GridSpacingX",QString("-1")).toDouble();
		userGrid.y = -1.;//RS_SETTINGS->readEntry("/GridSpacingY",QString("-1")).toDouble();
	}
	int minGridSpacing = 10;//RS_SETTINGS->readNumEntry("/MinGridSpacing", 10);
